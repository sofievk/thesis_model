...
% % Initial energy demand GHKT:
% pi00 = 1;               %Base period share of labor devoted to final goods production    
%     E1_2008 = (3.43+1.68)/0.0676;       %GtC converted to TWh per annum
%     E2_2008 = 3.75/0.1008;              %GtC per annum
%     E3_2008 = (1.95/0.1008)/5.87;       %GtC-eq per annum
...
gZa_coal = 0.01;                                         %GHKT Coal labour productivity
...
%%Oil%%
%%%%%%%
R0 = 253.8/0.0676;                % GHKT
%R0 = 2720;                         % Recalculated based on reserves for 2024
...
gA = 0.00;
gAd = (1+gAa_y)^10-1; 
A(1) = (eta_GDP*(Y2024*10)) / Yt1_model;
for i = 1:1:T-1; 
    A(1+i) = A(i)*(1+gAd);
end
...
vars = 2*T+3*(T-1);         %Without mineral constraints
...
%load('x_effE','x');                              % (5) Changing ex
%load('x_enK_effE','x');                          % (6) Changing en_K and ex
%load('x_enK_effE_dK','x');                       % (7) Changing en_K, ex, kappaM
%load('x_enK_effE_dkdg','x');                     % (8) Changing en_K, ex, kappaM & delta_G
%load('x_enKy_effE_dkdg','x');                    % (9) Changing en_K, ex, kappaM, delta_G and Delta
...
%      x0(2*(T-1)+2*T+i) = M0-((M0/1.1)/T)*i; %mineral stock
% end
% x0(2*(T-1)+T) = 0.002;
% x0(2*(T-1)+T+T) = 0.01;
...
f = newces_Objective(x0,A,B,A2t,A3t,Delta,Delta_G,en_K,ex,G0,eta_GDP,K0,M0,N,R0,S1_2000,S2_2000,Sbar,T,alpha,beta,gA,gAd,gZ_coal,gZ_green,gZd_y,gZBGP,gamma,kappaL,kappaM,kappa1,kappa2,kappa3,kappa_capacity, kappa_supply,phi,phi0,phiL,phi_m,psi,rho,rho_E3,rho_energy,sigma,ypsilon);
[c, ceq] = newces_Constraints(x0,A,B,A2t,A3t,Delta_G,Delta,en_K,ex,G0,eta_GDP,K0,M0,N,R0,S1_2000,S2_2000,Sbar,T,alpha,beta,gA,gAd,gZ_coal,gZ_green,gZd_y,gZBGP,gamma,kappaL,kappaM,kappa1,kappa2,kappa3,kappa_capacity, kappa_supply,phi,phi0,phiL,phi_m,psi,rho,rho_E3,rho_energy,sigma,ypsilon);
...
[x, fval,exitflag] = fmincon(@(x)newces_Objective(x,A,B,A2t,A3t,Delta,Delta_G,en_K,ex,G0,eta_GDP,K0,M0,N,R0,S1_2000,S2_2000,Sbar,T,alpha,beta,gA,gAd,gZ_coal,gZ_green,gZd_y,gZBGP,gamma,kappaL,kappaM,kappa1,kappa2,kappa3,kappa_capacity, kappa_supply,phi,phi0,phiL,phi_m,psi,rho,rho_E3,rho_energy,sigma,ypsilon), x0, [], [], [], [], lb, ub, @(x)newces_Constraints(x,A,B,A2t,A3t,Delta,Delta_G,en_K,ex,G0,eta_GDP,K0,M0,N,R0,S1_2000,S2_2000,Sbar,T,alpha,beta,gA,gAd,gZ_coal,gZ_green,gZd_y,gZBGP,gamma,kappaL,kappaM,kappa1,kappa2,kappa3,kappa_capacity, kappa_supply,phi,phi0,phiL,phi_m,psi,rho,rho_E3,rho_energy,sigma,ypsilon), options);
...
%save('x_effE','x');                              % (5) Changing ex
%save('x_enK_effE','x');                          % (6) Changing en_K and ex
%save('x_enK_effE_dK','x');                       % (7) Changing en_K, ex, kappaM
%save('x_enK_effE_dkdg','x');                     % (8) Changing en_K, ex, kappaM & delta_G
%save('x_enKy_effE_dkdg','x');                    % (9) Changing en_K, ex, kappaM, delta_G and Delta
%save('x_ces_cd','x');                             % (10) CES rho =0.05

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%      Section 3: Compute Allocations and Carbon Taxes  %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%% Section 3A: OPTIMAL TAX SCENARIO %%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%   Energy Production   %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Yt(1) = (exp((-gamma(1))*(St(1)-Sbar)))*(min(en_K(1)*K0,ex(1)*energy(1))^alpha)*(((1-x(2*(T-1)+1)-x(2*(T-1)+T+1))*N)^(1-alpha));
    %Yt(1) = (min(en_K(1)*K0,ex(1)*energy(1))^alpha)*(((1-x(2*(T-1)+1)-x(2*(T-1)+T+1))*N)^(1-alpha));
...
%%Option 2 PF Nested CES: 
    ...
    Yt(1) = A(1)*(exp((-gamma(1))*(St(1)-Sbar)))*(U(1)^alpha)*(((1-x(2*(T-1)+1)-x(2*(T-1)+T+1))*N)^(1-alpha));
        %LF: 
         %Yt(1) = A(1)*(U(1)^alpha)*(((1-x(2*(T-1)+1)-x(2*(T-1)+T+1))*N)^(1-alpha));
...
%Yt(1+i) = (exp((-gamma(1+i))*(St(1+i)-Sbar)))*(min(en_K(1+i)*Kt1(i),ex(1+i)*energy(1+i))^alpha)*(((1-x(2*(T-1)+1+i)-x(2*(T-1)+T+1+i))*N)^(1-alpha));
    %Yt(1+i) = (min(en_K(1+i)*Kt1(i),ex(1+i)*energy(1+i))^alpha)*(((1-x(2*(T-1)+1+i)-x(2*(T-1)+T+1+i))*N)^(1-alpha));
...
%%Option 2 PF Nested CES: 
    ...
    Yt(1+i) = A(1+i)*(exp((-gamma(1+i))*(St(1+i)-Sbar)))*(U(1+i)^alpha)*(((1-x(2*(T-1)+1+i)-x(2*(T-1)+T+1+i))*N)^(1-alpha));   
      %LF:
      %Yt(1+i) = A(1+i)*(U(1+i)^alpha)*(((1-x(2*(T-1)+1+i)-x(2*(T-1)+T+1+i))*N)^(1-alpha));   
...
%     %Yt(T) =  (min(en_K(T)*Kt1(T-1),ex(T)*energy(T))^alpha)*(((1-x(2*(T-1)+T)-x(2*(T-1)+2*T))*N)^(1-alpha));
...
%%Option 2 PF Nested CES: 
    ...
    Yt(T) = A(T)*(exp((-gamma(T))*(St(T)-Sbar)))*(U(T)^alpha)*(((1-x(2*(T-1)+T)-x(2*(T-1)+2*T))*N)^(1-alpha));   
        %LF:
        %Yt(T) = A(T)*(U(T)^alpha)*(((1-x(2*(T-1)+T)-x(2*(T-1)+2*T))*N)^(1-alpha));   
...
%%Option 1 PF Leontief:
        ...
        % %Ytn(i) = (min(en_K(T)*Ktn(i),ex(T)*En(i))^alpha)*(((1-x(2*(T-1)+T)-x(2*(T-1)+2*T))*N)^(1-alpha));     
...
%%Option 2 PF Nested CES: 
        ...
        Ytn(i) = A(T)*(exp((-gamma(T))*(St(T)-Sbar)))*(Un(i)^alpha)*(((1-x(2*(T-1)+T)-x(2*(T-1)+2*T))*N)^(1-alpha));   
            %LF:
            %Ytn(i) = A(T)*(Un(i)^alpha)*(((1-x(2*(T-1)+T)-x(2*(T-1)+2*T))*N)^(1-alpha));   
...
%%% Diagnostic plot: Ratio of capital capacity / energy supply
capacity_k = zeros(T,1);
supply_e = zeros(T,1);
ratio = zeros(T,1);

capacity_k(1) = en_K(1)*K0;
supply_e(1) = ex(1)*energy(1);
ratio(1) = capacity_k(1)/supply_e(1);
for i = 1:1:T-1
capacity_k(1+i) = en_K(i+1)*Kt1(i);
supply_e(1+i) = ex(i)*energy(i);
...
ylabel('Ratio');
grid off;
title('Figure A8: (Ek*K) / (ex*E)');

%%% Diagnostic plot: Adjusted Leontief ratio (with kappa)
...
% Period 1 (uses K0)
cap_eff(1) = en_K(1) * K0 * (kappa_capacity(1)^(1/rho_energy));
sup_eff(1) = ex(1) * energy(1) * (kappa_supply(1)^(1/rho_energy));
ratio_eff(1) = cap_eff(1) / sup_eff(1);

% Periods 2:T (use Kt1 and energy(i))
for i = 1:T-1
    cap_eff(i+1) = en_K(i+1) * Kt1(i) * (kappa_capacity(1)^(1/rho_energy));
    sup_eff(i+1) = ex(i+1) * energy(i+1) * (kappa_supply(1)^(1/rho_energy));
    ratio_eff(i+1) = cap_eff(i+1) / sup_eff(i+1);
end

%%% Plot EXACTLY like your previous ratio figure
figure;
hold on;
plot(1:T, ratio_eff, '-o');
xlabel('period');
ylabel('Ratio');
title('Figure A8 (Adjusted): (kappa-adjusted capital) / (kappa-adjusted energy)');
grid off;
...
Kt1_current_run = Kt1;
save('Kt1_current_run', 'Kt1_current_run');
% Gt1_current_run = Gt1;
...
U_current_run = U;
save('U_current_run', 'U_current_run');
...
%Savings rate
load('r_current_run','r_current_run')

z = 25;
figure;
plot(y2(1:z), r_current_run(1:z), ' -b', 'LineWidth', 1.5);
xlabel('Year', 'FontSize', 11);
ylabel('Savings rate', 'FontSize', 11);
...
yyaxis right
plot(y2(1:z), temp_current_run(1:z), '-r', 'LineWidth', 1.5);
ylabel('Temperature Increase (degrees C)', 'FontSize', 11);
ylim([0 4])
...
hold on;
plot(y2(1:z), N2_current_run(1:z), ' -b', 'LineWidth', 1.5);
plot(y2(1:z), N3_current_run(1:z), ' -g', 'LineWidth', 1.5);
hold off; 
xlabel('Year', 'FontSize', 11);
ylabel('Share', 'FontSize', 11);
title('Labour Shares E2 and E3');
...
%% Stacked chart: C, K, GDP, savings rate
load('r_current_run.mat','r_current_run')
load('Ct_current_run.mat','Ct_current_run')
load('Kt1_current_run.mat','Kt1_current_run')
load('gdp_current_run.mat','gdp_current_run')

z = length(r_current_run);

figure; hold on

%% Left axis: C + K + GDP
yyaxis left
h = area(y2(1:z), [Ct_current_run(1:z) Kt1_current_run(1:z)], 'LineStyle','none');
h(1).FaceAlpha = .5; h(2).FaceAlpha = .5;
h(1).FaceColor = [0.2 0.6 0.8]; 
h(2).FaceColor = [0.8 0.4 0.4];

p = plot(y2(1:z), gdp_current_run(1:z), 'k--', 'LineWidth', 1.5);

ylabel('GDP, Consumption, Capital (billion USD)')
ylim([0 850000])

%% Right axis: savings rate
yyaxis right
s = plot(y2(1:z), r_current_run(1:z), '--', 'LineWidth', 1.5);
...
legend([h(1) h(2) p s], ...
    'Consumption','Capital','GDP','Savings rate', ...
    'Location','southeast');
...
load('energy_current_run.mat','energy_current_run')
load('gdp_current_run','gdp_current_run')
load('U_current_run.mat','U_current_run')
...
%% Right axis: Energy + U
yyaxis right
plot(y2(1:z), energy_current_run(1:z), 'Color', [0.85 0.33 0.10], 'LineWidth', 1.5); hold on;
plot(y2(1:z), U_current_run(1:z), '--k', 'LineWidth', 1.5);
ylabel('Energy / U', 'FontSize', 11);
ylim([0 400])

xlabel('Year', 'FontSize', 11);
title('GDP, Energy Use, and Effective Energy (U)');
xlim([2010 2225]);

legend('GDP','Energy','U','Location','best');
grid off;
%----------------------------------------------------------------------
...
%                     (min(en_K(1)*K0_try, ex(1)*energy(1))^alpha_try) * ...
...
%                         (min(en_K(i)*Kt1_try(i-1), ex(i)*energy(i))^alpha_try) * ...
